...
Slug derivation re-exports the canonical helper from `${CLAUDE_SKILL_DIR}/../scope-changeset/scripts/changeset_scope.py`. The slug:
...
- Every script under `${CLAUDE_SKILL_DIR}/scripts/` dispatches through the `thread_store` facade; concrete backend modules are never imported directly by skill code
