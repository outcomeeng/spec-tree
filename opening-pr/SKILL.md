...
7. **NEVER `gh pr checks --watch`** — for CI status, surface a single `gh pr checks` or `gh run view` and stop. Polling brings the machine to a halt given the huge number of agents waiting for PR checks.
