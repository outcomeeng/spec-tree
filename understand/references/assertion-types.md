<overview>
The five assertion types classify an assertion under the **testing** verification type, read from the assertion's quantifier. Evaluating (`[eval]`) and auditing (`[audit]`) carry no assertion type. `/test` (with `/test-{language}`) selects the verification type first — see `references/verification-kinds.md` — then, under testing, the assertion type. This reference defines the assertion types; it does not select them.
...
Only a `test` assertion carries an assertion type (one of the five); `evaluating` and `audit` carry none.
