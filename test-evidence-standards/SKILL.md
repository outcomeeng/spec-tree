...
6. Is any value at the call site one the assertion does not state?

Incidental values only the test needs come from generators over variable domains or from harness handles.
...
**Worked adapter example.** An adapter over an external tool has a two-sided state boundary, and question 3 applies to every expectation on both sides throughout the node:

- **Request side.** Conformance evidence reads the tool's captured usage text by path as an inert fixture and requires every emitted subcommand and flag to appear in that tool surface; the tool surface owns the oracle. An expected argument vector built by the subject under test or by a copied builder fails question 3. Mapping evidence derives the request-to-argv correspondence from the governing spec's declared mapping or a source-owned registry, never from an adapter builder and never by parsing the captured usage text; parsing help text to construct the expected argument vector is a second implementation, not an independent mapping law.
- **Response side.** Expected envelopes, inventories, and errors come from captured real tool output read by path, covering success and a reproducible error condition such as a nonexistent socket. An expected envelope composed from adapter field constants or hand-typed fields fails question 3.

A variant of a captured artifact changes only the value the assertion's domain ranges over, and its name states which capture it varies. Every captured tool artifact used as an oracle names the tool and version in the fixture, and the governing node states its recapture condition: when the version pin the adapter reads changes, or when a probe reports drift. A capture from an older release is a stale oracle, not evidence.
